...
function Editor() {
  ...
  const [vercelResult, setVercelResult] = useState<{ url: string } | { error: string } | null>(
    null,
  );
  const [githubPushing, setGithubPushing] = useState(false);
  const [githubResult, setGithubResult] = useState<{ repoUrl: string } | { error: string } | null>(
    null,
  );
  ...
  const send = async () => {
    ...
    try {
      ...
      if (isLimitError(e)) {
        setMessages((m) => m.slice(0, -2)); // remove the "you" + "working" messages
        setShowUpgrade(true);
      } else {
        setMessages((m) => [
          ...m.slice(0, -1),
